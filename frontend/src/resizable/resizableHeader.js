...
const Resizer = ({ resizeHandler, id, setResizing, isZeroSum, resizerWidth = 4 }) => {
  return (
    <div
      style={{
...
const Cell = ({ defaultWidth = 'maxWidth', resizedWidth, resizeHandler, setBasisWidth, id, setResizing, isZeroSum }) => {
  ...
  return (
    <div
      ...
      className={`relative border-b border-r bg-hermit-grey-900 text-hermit-grey-400 border-hermit-grey-400 h-6 px-2 font-bold`}
    >
      {id}
...
const Header = ({ columnConfigs = colConfigs, maxWidth = 600 }) => {
  // const [resizing, setResizing] = useState(assoc('order')
  ...
  return (
    <div
      className={`flex border-l border-t border-hermit-grey-400`}
    >
      {
        map(config =>
              <Cell 
...
export { Header as ResizableHeader, resizingAtom }